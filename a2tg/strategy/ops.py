...
from a2tg import quat_scale_to_covar_preci
from a2tg.relocation import compute_relocation
from a2tg.utils import normalized_quat_to_rotmat
...
def split(
    ...
    def param_fn(name: str, p: Tensor) -> Tensor:
        ...
        if name == "means":
            ...
            if isotropic:
                new_scales = scales[..., 0:1]
            else:
                new_scales = scales
            p_split = torch.log(new_scales / 1.6).repeat(2, 1)  # [2N, 3]
...
def relocate(
    ...
    if is_wsr:
        opacities = torch.tanh(params["opacities"])
        neg_mask = torch.where(opacities < 0.0, -1, 1)
    else:
    ...
    isotropic = params["scales"].size(1) == 1
    ...
    if is_wsr:
        new_opacities = new_opacities * neg_mask[sampled_idxs]

    def param_fn(name: str, p: Tensor) -> Tensor:
        nonlocal new_scales
        if name == "opacities":
            if is_wsr:
                p[sampled_idxs] = torch.arctanh(new_opacities)
            else:
                p[sampled_idxs] = torch.logit(new_opacities)
        elif name == "scales":
            if isotropic:
                new_scales = new_scales[..., 0:1]
