...
__global__ void rasterize_to_pixels_bwd_packed_textured_gaussians_kernel(
    ...
    at::PackedTensorAccessor32<S, 2, at::RestrictPtrTraits> v_textures_packed,  // [C, N, TEXTURE_DIM]
...
call_kernel_with_dim(
    ...
    const torch::Tensor &textures_packed,  // 
    const torch::Tensor &texture_dims,  // 
    const torch::Tensor &texture_offsets,  // 
    ...
    GSPLAT_CHECK_INPUT(textures_packed);
    GSPLAT_CHECK_INPUT(texture_dims);
    GSPLAT_CHECK_INPUT(texture_offsets);
    ...
    torch::Tensor v_textures_packed = torch::zeros_like(textures_packed);
